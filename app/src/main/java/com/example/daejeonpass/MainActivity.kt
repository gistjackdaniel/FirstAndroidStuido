...
fun AppNavigation(navController: NavHostController, modifier: Modifier = Modifier, reviewViewModel: ReviewViewModel,
    ...
    NavHost(
        ...
        composable(
            "detail/{postId}",
            arguments = listOf(navArgument("postId") { type = NavType.IntType }) // postId 타입 명시
        ) { backStackEntry ->
            val postId = backStackEntry.arguments?.getInt("postId") // Int로 받음
            // PostRepository에서 post를 가져오거나, ViewModel을 통해 가져오는 것이 더 좋음
            val post = PostRepository.posts.find { it.id == postId }
            ...
            if (post != null) {
                CardDetail(
                    post = post,
                    onBookmarkClick = {
                        PostRepository.toggleBookmark(post.id) // 예시: Repository 직접 호출
                        // UI 업데이트를 위해서는 State를 사용해야 함
                    },
                    navController = navController,
                ...
                Text("게시물을 찾을 수 없습니다.")
                LaunchedEffect(Unit) { // 잠시 후 뒤로 가기
                    delay(2000)
